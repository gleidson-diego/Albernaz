...
public class EventoEmailFinalizacaoOP implements EventoProgramavelJava {

    private static final BigDecimal COD_MODELO_EMAIL = BigDecimal.valueOf(4);
    ...
    private static final String SQL_BUSCA_IDIATV_POR_IDIPROC =
            "SELECT MIN(ATV.IDIATV) AS IDIATV " +
                    "  FROM TPRIATV ATV " +
    ...
    public void beforeUpdate(PersistenceEvent event) throws Exception {
        ...
        final String newStatus = safeStr(newVO.asString("STATUSPROC"));
        final String oldStatus = (oldVO != null ? safeStr(oldVO.asString("STATUSPROC")) : "");


        if (!"F".equalsIgnoreCase(newStatus)) return;
        if ("F".equalsIgnoreCase(oldStatus)) return;

        final BigDecimal idiproc = newVO.asBigDecimalOrZero("IDIPROC");
        if (idiproc == null || idiproc.compareTo(BigDecimal.ZERO) == 0) return;

        // Anti-duplicidade
        final String flagKey = "BHZ_EMAIL_FINALIZA_OP_IDIPROC_" + idiproc;
        ...
        try {
            ...
            final BigDecimal idiAtv = buscarIdiAtvPorIdiproc(idiproc);
    ...
    private BigDecimal buscarIdiAtvPorIdiproc(BigDecimal idiproc) throws Exception {
        ...
        try {
            ...
            sql.appendSql(SQL_BUSCA_IDIATV_POR_IDIPROC);
            sql.setNamedParameter("IDIPROC", idiproc);

            rs = sql.executeQuery();
            if (rs.next()) {
                return rs.getBigDecimal("IDIATV");
            }
            return null;
    ...
    private void dispararEmailComAnexo(BigDecimal idiAtv, BigDecimal usuRemet) throws Exception {
        ...
        try {
            ...
            NativeSql sql = new NativeSql(jdbc);
            sql.loadSql(EventoEmailFinalizacaoOP.class, SQL_DADOS);
